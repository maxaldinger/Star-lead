...
async function fetchNews(company: string): Promise<string[]> {
  try {
    const url = `https://news.google.com/rss/search?q=${encodeURIComponent(company + ' data AI')}&hl=en-US&gl=US&ceid=US:en`
...
export async function POST(req: Request) {
  ...
  try {
    const msg = await client.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `You are an enterprise account intelligence analyst for Stardog, a knowledge graph platform that unifies siloed enterprise data without moving it, enabling AI applications grounded in verified data relationships.

Analyze this company for Stardog sales targeting:
...
Return ONLY a JSON object:
{
  ...
  "hq": "City, State",
  "primary_vertical": "Financial Services|Healthcare|Manufacturing|Energy|Government|Technology",
  "relevance_score": 0-100,
  "relevance_label": "Prime Target|Strong Fit|Moderate Fit|Low Priority",
  "relevance_color": "green|yellow|orange|red",
  "snapshot": "3-4 sentence company overview focused on data landscape",
  "stardog_fit": "2-3 sentences on how Stardog specifically solves their data challenges",
  "data_challenge": "2-3 sentences on their core data integration problems",
  "signals": [
    { "type": "news|funding|hiring|contract|partnership|earnings|research|regulation", "urgency": "high|medium|low", "text": "Signal description", "date": "YYYY-MM" }
  ],
  "target_contacts": [
    { "title": "Job Title", "department": "Department", "why_target": "Why this person", "linkedin_search": "search query" }
  ],
  "outreach_angle": "2-3 sentence outreach hook",
  "email_subject": "Under 8 words",
  "talking_points": ["point 1", "point 2", "point 3"],
  "competitor_risk": "Neo4j, Denodo, Palantir, or other competitors likely involved",
  "risk_flags": ["flag1", "flag2"]
}

Scoring: green=80-100 (clear data unification need + active signals), yellow=60-79, orange=40-59, red=0-39.
Include 4-6 signals and 4 target contacts. Return ONLY the JSON object.`
      }]
    })
